      ...
      <div>
        <q-card v-if="event" style="max-width: 300px;">
          ...
          <q-card-actions align="center" class="bg-accent q-pb-md pb-mobile">
            ...
            <q-btn label="刪除預約" color="negative" @click="delReservations(event._id, event.reservation)"></q-btn>
          </q-card-actions>
        </q-card>
      </div>
    </q-dialog>
      ...
      <template #day="{ timestamp }">
        <template v-for="(event, index) in getEvents(timestamp.date)">
          ...
          >
            ...
            <span class="ellipsis">{{ event.reservation.name }}</span>
            <span v-if="event.reservation.gender === 'male'">先生</span>
            <span v-else-if="event.reservation.gender === 'female'">小姐</span>
          </q-badge>
          <q-badge
            :key="index"
            v-if=" event.title === '已取消預約'"
            ...
            @click.stop.prevent="showEvent(event)"
          >
            {{ event.reservation.time }}{{ event.reservation.name }}
...
export default {
  ...
  methods: {
    ...
    async delReservations (_id) {
      console.log(_id)
      try {
        await this.api.delete('/Reservations/' + _id, {
          headers: {
            authorization: 'Bearer ' + this.$store.getters['user/user'].token
          }
        })
